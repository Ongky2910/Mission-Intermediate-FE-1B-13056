...
const Profile = () => {
  ...
  const { handleSubscriptionUpdate, handleCancelSubscription } =
    useSubscription(); // Mengambil fungsi dari konteks langganan
  ...
  const usernameRef = useRef(null); // Membuat ref untuk input nama pengguna
  ...
  useEffect(() => {
    if (selectedAccount) {
      ...
      setPackageType(selectedAccount.packageType);
      setIsSubscribed(selectedAccount.isSubscribed);
      setAvatar(selectedAccount.avatar || "src/assets/27470334_7309681.jpg");
    }
  }, [selectedAccount]);
  ...
  const saveProfile = () => {
    ...
    setTimeout(() => {
      const newAccount = { username, email, packageType, isSubscribed, avatar }; // Akun yang baru atau yang diperbarui
  
      // Cek apakah akun yang sedang diedit ada
      const updatedAccounts = savedAccounts.map((account) => {
        if (account.username === selectedAccount?.username) {
          return newAccount; // Memperbarui akun yang ada
        }
        return account; // Kembalikan akun yang tidak diedit
      });
  
      // Jika tidak ada akun yang sedang diedit, tambahkan yang baru
      if (!selectedAccount) {
  ...
  const editAccount = (account) => {
    ...
    setSelectedAccount(account); // Menyimpan akun yang sedang diedit

    // Arahkan fokus ke input pertama
  ...
  const handleSubscribe = () => {
    ...
    const updatedAccounts = savedAccounts.more((account) =>
      account.username === selectedAccount.username
        ? { ...account, isSubscribed: true, packageType: selectedPackage }
        : account
    );
  ...
  const handleCancel = () => {
    ...
    const updatedAccounts = savedAccounts.map((account) =>
      account.username === selectedAccount.username
        ? { ...account, isSubscribed: false, packageType: "" }
        : account
    );
  ...
  const deleteAccount = (accountToDelete) => {
    ...
    if (
      selectedAccount &&
      selectedAccount.username === accountToDelete.username
    ) {
              ...
              >
                <div>
                  ...
                  <p className="text-gray-400">
                    Paket: {account.packageType || "Belum Berlangganan"}
                  </p>
                  <p className="text-gray-400">
                    Status:{" "}
